...
def run_work(db, id=None):
    ...
    for rn in rn_list:
        ...
        log.info('开始运行子进程:id[%d] script[%s] start[%s] end[%s] over[%s] status[%s]' % (row['id'], script, row['start_date'], row['end_date'], row['over_date'], row['status']))
...
def run_work_process(db, row):
    ...
    notice(options, over_status, '连续跑批脚本执行完成[%s][%s]' % (scriptName, data_date), 
                                 '连续跑批脚本执行失败[%s][%s]' % (scriptName, data_date))
...
# 通知信息发送
def notice(options, over_status, succ_msg=None, fail_msg=None):
    ...
    if not msg:
        log.info('不需要发送信息:error_notice[%s] over_notice[%s] [%s]' % options['error_notice'], options['over_notice'], over_status)
        return
    ...
    for receiver in notice_receiver:
        ...
        if m:
            log.info('发送短信:[%s][%s]' % (receiver, msg))
            cmd = 'curl http://11.168.35.34:8101/uassService/uassserviceaction.action -d uaap_request_result=json -d _fw_service_id=sendSms -d userName=TS150 -d handSet=%s -d opName=SMS -d operator=TS150 -d smsContent="%s"' % \
                    (receiver, msg)

        m = patt_email.match(receiver)
        if m:
            log.info('发送邮件:[%s][%s]' % (receiver, msg))
            cmd = 'curl -s http://11.168.35.34:8101/uassService/uassserviceaction.action -d uaap_request_result=json -d _fw_service_id=sendMail -d userName=TS150 -d mailbox=%s -d opName=SMS -d operator=TS150 -d mailContent="%s" -d mailTitle="%s"' % \
                    (receiver, msg, '安全监控组件告警')

        if cmd:
            # 运行脚本
            (returncode, out_lines) = executeShell(cmd)
            log.info('通知信息发送完成:[%s][%d]' % (cmd, returncode))
...
def daemon():
    ...
    while True:
        ...
        while True:
            ...
            if len(over_job_list) >= 1 or len(job_list) == 0:
                break
...
def list_work(*args, **kwargs):
    ...
    print('ID  %-23s%s%s %s %s %s    %s' % ('时间戳', '脚本名'.ljust(max_script_len + 4), '起始日期', '终止日期', '完成日期', '处理状态', '进程号'))
    for rn in rn_list:
        row = row_map[rn]
        options = row['options']
        script = options['script'].ljust(max_script_len + 1)
        print('%-4d%-20s%s%-9s%-9s%-9s%-12s%s' % (row['id'], row['ts'], script, row['start_date'], row['end_date'], row['over_date'], row['status'], row['pid']))
        # print row
    exit(0)
...
def main():
    ...
    if options.script:
        ...
        if not options.crontab:
            ...
            print cron_time
            sched.db.save_crontab_config(crontab=options.crontab, script=scriptName, options=options)

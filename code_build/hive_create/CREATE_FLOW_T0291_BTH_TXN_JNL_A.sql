...
-- 外部表
DROP TABLE IF EXISTS EXT_T0291_BTH_TXN_JNL_A;

CREATE EXTERNAL TABLE IF NOT EXISTS EXT_T0291_BTH_TXN_JNL_A(
    ...
    ACQ_DCC_US_ST             string comment '收单DCC使用状态',
    PERMRCHACQDCCBHDCGAMT     string comment '单笔商户收单动态币种转换业务手续费金额',
    ...
    TMNL_BTNO                 string comment '终端批次号',
    ...
    SBJ_CNVS_ID               string comment '科目转换编号',
    CLMN_SN                   string comment '栏位序号',
...
-- ORC内部表，节约存储空间
DROP TABLE IF EXISTS INN_T0291_BTH_TXN_JNL_A;

CREATE TABLE IF NOT EXISTS INN_T0291_BTH_TXN_JNL_A(
    POS_OFSTBAL_DT            string comment 'POS轧账日期',
    CLRG_TXN_CD               string comment '清算交易码',
    TXN_CD_CHN_NM             string comment '交易代码中文名称',
    CARDNO                    string comment '卡号',

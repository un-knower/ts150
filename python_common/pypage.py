...
def lex(src):
    ...
    open_delims = {}
    for t in tagNodeTypes:
        open_delims[t.open_delim] = t
...
def pypage_from_file(source_file, seed_env=dict(), output_file=None):
    if os.path.exists(source_file):
        with open(source_file, 'r') as source_file:
            source = source_file.read()
    else:
        print("File %s does not exist." % repr(source_file), file=sys.stderr)
        sys.exit(1)

    output = pypage(source, seed_env)
...
def main():
    ...
    try:
        ...
        if args.data:
            ...
            data = {"table_en":"toddc", "table_cn":"核心系统表", "field_list":[
                 ('f1', '字段1'), ('f2', '字段2'), ('f3', '字段2'), ('f4', '字段2'), ('f5', '字段2'), ('f6', '字段2')]}
        pe = PypageExec(data)

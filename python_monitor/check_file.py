...
def hdfs_file(store_server, file_path):
    if store_server == 'ts150_hadoop':
        # 判断登录状态
        # klist
        (return_code, out_lines, err_lines) = execute_command('klist')
        

    if store_server == 'p9_hadoop':
        # 判断登录状态
        # klist
    ...
    if return_code != 0:
        return (cmd_text, return_code, out_res)
    ...
    for line in out_lines:
        ...
        if len(field_array) > 5:
            ...
            out_res.append([file_permission, file_size, file_mtime, file_path, filestatus])

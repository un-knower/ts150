...
def get_struct_time(time_stamp_int=None):
    """
...
def isdo(strs, tips=None):
    ...
    try:
        ...
        now = time.strftime("%Y%m%d%H%M00", time.localtime())
        time_stamp = time.strptime(now, "%Y%m%d%H%M00")
...
def nowisdo(conf_string):
    """
    判断当前时间是否与配置串吻合！
    """
    res, cron_time = parse_crontab_time(conf_string)
    if res != 0:
        return False
    ...
    #time_stamp = int(time.time())
    #解析 当前时间戳对应的 分 时 日 月 周
    time_struct = get_struct_time()
    match_res = time_match_crontab(cron_time, time_struct)

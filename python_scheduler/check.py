...
# GP SQL 执行
def psql(schema, table_name, sql, logFile='check_gp'):
...
# 获取GP表属性
def get_gp_table_attribute(table, check_date=None, logFile='check_gp'):
    log.info('开始运行--获取GP表属性:[%s][%s]' % (table, check_date), logFile)
    ...
    # 表是否存在
    table_rows = psql(schema, table_name, "select * from pg_tables where schemaname='%s' and tablename='%s'" % (
        schema, table_name), logFile=logFile)
    if len(table_rows) == 1:
        # 表是否分区
        # partition_rows = psql(schema, table_name, "select * from pg_partitions where schemaname='%s' and tablename='%s'" % (
        #     schema, table_name), logFile=logFile)
        ...
        #     partition_column_rows = psql(schema, table_name, sql, logFile=logFile)
        #     date_field_name = partition_column_rows[0]['columnname']
        pass
    else:
        # 视图是否存在
        view_rows = psql(schema, table_name, "select * from pg_views where schemaname='%s' and viewname='%s'" % (schema, table_name), logFile=logFile)
        if len(view_rows) == 0:
            return (False, check_date, 0)

    log.info('找到GP表或分区:[%s][%s][%s]' % (table, is_partition_table, date_field_name), logFile)
    ...
    if not check_date:
        ...
        if is_partition_table:
            ...
            max_partition_rows = psql(schema, table_name, sql, logFile=logFile)
            ...
            max_date_rows = psql(schema, table_name, sql, logFile=logFile)
    ...
    log.info('确认GP表检查日期:[%s][%s]' % (table, check_date), logFile)
    ...
    if is_partition_table:
        ...
        partition_rows = psql(schema, table_name, sql, logFile=logFile)
        if len(partition_rows) == 1:
    ...
    count_rows = psql(schema, table_name, sql, logFile=logFile)

    log.info('完成--获取GP表属性:[%s][%s][%s]' % (table, check_date, count_rows[0]['count']), logFile)

    return (True, check_date, int(count_rows[0]['count']))


# GP表检查,支持表名后带表达式：record_num>100
def valid_gp_table(table, check_date, logFile='check_gp'):
    if ':' in table:
        patt = re.compile(r'([\.\w]+):\{(.*)\}')
        m = patt.match(table)
        if m:
    ...
    (hasTable, real_check_date, record_num) = get_gp_table_attribute(table, check_date, logFile=logFile)

    if not hasTable:
        log.info('GP表或分区不存在:[%s][%s]' % (table, check_date), logFile)
        return False

    try:
        # 检查条件脚本执行
        if not eval(express):
            log.info('表达式检查未通过:[%s][%s][%s]' % (table, check_date, express), logFile)
            return False
    except Exception as e:
        raise CommonError(msg="GP表检查表达式解释有误:%s,%s" % (express, e))

    log.info('GP表检查通过:[%s][%s][%s]' % (table, check_date, express), logFile)

    return True


# 检查HDFS文件是否存在
def has_hdfs_file(path, check_file_size=True, logFile='check_hdfs'):
    try:
        (processing, fileNumTotal, fileSizeTotal) = get_hdfs_file_attribute(path, logFile=logFile)
    except CommonError:
        return False
...
# 返回HDFS文件属性
def get_hdfs_file_attribute(path, logFile='check_hdfs'):
    fileSizeTotal = 0
    fileNumTotal = 0
    processing = False
    ...
    # print "==========================================="
    for i in range(1):
        log.info(cmd, logFile)
        retcode, outlines, errlines = execute_command(cmd, logFile=logFile)
        if retcode != 0:
            log.warning('hadoop fs命令执行失败:[%s][%d]' % (cmd, retcode), logFile)
            continue
        for line in outlines:
            # 目录不计入文件
        ...
        break
    # else:
    #     log.info('hadoop ls命令出错:%s' % cmd, logFile)
    #     raise CommonError(msg='hadoop ls命令出错:%s' % cmd)

    log.info('hdfs:[%s][%s][%s]' % (processing, fileNumTotal, fileSizeTotal), logFile)

    return (processing, fileNumTotal, fileSizeTotal)


# HDFS目录检查,支持目录名后带表达式：(file_num>1 and file_size>=1024) or True
def valid_hdfs_file(path, data_date=None, logFile='check_hdfs'):
    if ':' in path:
        patt = re.compile(r'(\S+):\{(.*)\}')
        m = patt.match(path)
        if m:
    ...
    try:
        ...
        (processing, file_num, file_size) = get_hdfs_file_attribute(path, logFile=logFile)
        ...
        log.warning("HDFS检查表达式解释有误:%s,%s" % (express, e), logFile)
...
def has_hive_table_partition(table, partition_value, check_file_size=True, check_record_num=False, logFile='check_hive'):
    ret = False
...
def valid_hive_table(table, partition_value=None, logFile='check_hive'):
    ret = False
...
# 获取Hive表记录数
def get_hive_record_num(table, where=None, logFile='check_hive'):
    cmd = 'beeline --outputformat=vertical -e "%s"'
    sql = "select count(*) as count_num from %s " % table
    if where:
        sql += "where %s" % where

    log.info(cmd % sql, logFile)
    retcode, outlines, errlines = execute_command(cmd % sql, logFile=logFile)
    for line in outlines:
        if 'count_num' in line:
            # print '[%s]' % line
            line_array = re.split(space_pattern, line)
            record_num = int(line_array[1])
            log.info('hive table:%s record_num:[%s]' % (table, record_num), logFile)

            return record_num


# 获取HIVE表或分区属性
def get_hive_table_attribute(table, get_record_num=False, partition_value=None, logFile='check_hive'):
    ...
    if not partition_value:
        ...
        for line in outlines:
            if 'Location:' == line[:9]:
                ...
                # log.info(hdfs_path, logFile)
                # print hdfs_path

                # 获取HDFS文件属性
                (processing, fileNumTotal, fileSizeTotal) = get_hdfs_file_attribute(hdfs_path, logFile=logFile)

                break

        # 状态正常，且需要查记录数
        if get_record_num and not processing and fileNumTotal > 0 and fileSizeTotal > 0:
            record_num = get_hive_record_num(table, logFile=logFile)

        ret_attribute_array.append((processing, fileNumTotal, fileSizeTotal, record_num))

    else:
        sql = "show partitions %s" % (table)
        log.info(sql, logFile)
        #retcode, outlines, errlines = execute_command(cmd % sql, logFile=logFile)
        retcode, outlines = executeShell(cmd % sql)
        ...
        for line in outlines:
            ...
            if len(partition_qry_array) > 0:
                ...
                log.info(sql, logFile)
                # print sql
                retcode, outlines2, errlines = execute_command(cmd % sql, logFile=logFile)
                ...
                # 状态正常，且需要查记录数
                if get_record_num and not processing and fileNumTotal > 0 and fileSizeTotal > 0:
                    record_num = get_hive_record_num(table, ' and '.join(partition_qry_array), logFile=logFile)
...
# 获取目录下对应的文件列表
def get_local_file_attribute(pathName, data_date=None, logFile='check_local'):
    log.info('获取本地文件属性:%s' % pathName, logFile)
    if isLinux():
        cmd = 'ls'
    else:
        cmd = 'dir'
    ...
    log.info('变更替换后本地文件属性:%s' % pathName, logFile)
    ...
    if retcode == 0:
        for line in outlines:
            ...
            log.info('本地文件%s属性:[%s][%d]' % (fileName, processing, stat.st_size), logFile)
    else:
        log.warning('命令[%s]执行失败' % cmd, logFile)
...
def valid_local_file(pathName, data_date=None, logFile='check_local'):
    ...
    if ':' in pathName:
        ...
        # 在导出文件时，做结果文件检查，一定会处理正在处理状态
        # express = 'file_num>=1 and file_size>=100 and processing==False'
        express = 'file_num>=1 and file_size>=100'

    ret_file_array = get_local_file_attribute(pathName, data_date, logFile=logFile)
    ...
    for (fileName, processing, file_size) in ret_file_array:
        try:
            ...
            # 检查条件脚本执行
            if not eval(express):
                log.warning('表达式[%s]检查没通过[%s][%s][%d][%d][%d]' % (express,
                    fileName, processing, file_size, file_num, total_file_size), logFile)
                return False
        except Exception as e:
            raise CommonError(msg="本地文件检查表达式解释有误:%s,%s" % (express, e))

    log.info('检查本地文件属性:[%s][%s]通过' % (pathName, data_date), logFile)

...
run()
{
   beeline -f $script_path/1_create_table/hive_insert/INSERT_EXT_T0042_TBPC1510_H.sql --hivevar log_date=${log_date}
   # ret=$?

   # 插入成功则删除HDFS源文件
   # if [ $ret -eq 0 ]; then
   #    hadoop fs -rm -r -f -skipTrash /bigdata/input/TS150/case_trace/T0042_TBPC1510_H
   # fi
   return $?
}

...
run()
{
   beeline -f $script_path/1_create_table/hive_insert/INSERT_EXT_TODDC_FCMTLR0_H.sql --hivevar log_date=${log_date}
   # ret=$?

   # 插入成功则删除HDFS源文件
   # if [ $ret -eq 0 ]; then
   #    hadoop fs -rm -r -f -skipTrash /bigdata/input/TS150/case_trace/TODDC_FCMTLR0_H
   # fi
   return $?
}
